...
export const generateRegistrationOptions = async (username: string) => {
  const response = await fetch(`${EXPO_PUBLIC_FLASH_API_BASE_URL}/accounts/v1/passkeys/registration/generate-options`, {
    method: 'POST',
    headers: {
...
export const verifyRegistration = async (registrationResponse: RegistrationResponseJSON): Promise<User> => {
  const response = await fetch(`${EXPO_PUBLIC_FLASH_API_BASE_URL}/accounts/v1/passkeys/registration/verify`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
export const generateAuthenticationOptions = async () => {
  const response = await fetch(`${EXPO_PUBLIC_FLASH_API_BASE_URL}/accounts/v1/passkeys/authentication/generate-options`, {
    credentials: 'include'
  });
  if (!response.ok) throw response;
  return response.json();
...
export const verifyAuthentication = async (authenticationResponse: AuthenticationResponseJSON): Promise<User> => {
  const response = await fetch(`${EXPO_PUBLIC_FLASH_API_BASE_URL}/accounts/v1/passkeys/authentication/verify`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
export const fetchTotalAPY = async () => {
  const response = await axios.get<number>(`${EXPO_PUBLIC_FLASH_ANALYTICS_API_BASE_URL}/analytics/v1/yields/total-apy`)
